...
import { calculateEventStatus, getStatusColor } from '../../utils/eventStatus.js';
import Badge from '../common/Badge.jsx';

const EventCard = ({ event, showActions = true, basePath = '/participant/events' }) => {
      ...
      <div className="p-4">
        ...
        <div className="space-y-1.5 text-sm text-neutral-600 mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="truncate">{formatDate(event.startDate)} • {event.startTime}</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {event.organizer && (
            <div className="text-right">
              ...
              <div className="text-sm font-medium text-neutral-700 truncate max-w-[120px]">
        ...
        {showActions && (
          <div className="flex gap-2">
            ...
            {isRegistered && basePath.includes('participant') ? (
              userRegistration._id ? (
                <Link
                  to={`/participant/registrations/${userRegistration._id}/ticket`}
                  className="flex-1 btn-primary text-sm py-1.5"
                >
                  {userRegistration.status === 'Waitlisted' ? 'View Waitlist' : 'View Ticket'}
                </Link>
              ) : null
            ) : canRegister && isFull && event.allowWaitlist ? (
              <Link
                to={`${basePath}/${event._id}/register`}
                className="flex-1 btn-primary text-sm py-1.5"
              >
                Join Waitlist
              </Link>
